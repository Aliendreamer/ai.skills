# Spec: llm-setup-audit

## Purpose

Defines what the LLM setup audit skill must check when reviewing a project's Claude Code
configuration for correctness and security. The audit covers committed settings, secrets
hygiene, and cross-references runtime behavioral rules from related skills.
...
- **THEN** the audit SHALL report FLAG and recommend moving the value to gitignored
  `settings.local.json`
