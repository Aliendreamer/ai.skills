...
Hold the repo's authored markdown (skills, prompts, specs, CLAUDE.md) to a committed style and a
zero-warning gate, so docs stay consistent without hand-policing.
...
and exclude vendored `.claude/**` content and `openspec/changes/**`.
...
- **THEN** files under `.claude/` and `openspec/changes/` are not reported
