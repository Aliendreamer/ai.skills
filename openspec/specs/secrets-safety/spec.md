...
Defines the behavioral rules agents MUST follow when operating in projects that have a
designated secrets source. These rules apply at script-writing and command-execution time —
they are not a replacement for filesystem sandbox restrictions, which serve only as a
structural fallback.
...
- **THEN** the agent SHALL write the command with a placeholder and instruct the user to
  substitute the value themselves
...
- **THEN** the agent SHALL write the full command with the credential as a placeholder and
  prompt the user to run it
...
- **THEN** the agent SHALL include `-s -o /dev/null` or equivalent to prevent the response
  from leaking into context
