# Coverage build of <AppName>.Backend for the instrumented E2E run: publish the app, then run it
...
WORKDIR /src
COPY <AppName>.Backend.csproj Directory.Build.props Directory.Packages.props ./
...
RUN for i in 1 2 3 4 5; do \
      ...
      dotnet restore <AppName>.Backend.csproj --disable-parallel && break; \
...
RUN dotnet publish <AppName>.Backend.csproj -c Release -o /app /p:UseAppHost=false /p:DebugType=portable
