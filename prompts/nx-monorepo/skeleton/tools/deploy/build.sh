#!/usr/bin/env bash
# Build one <AppName> app image (and, by default, push it to ACR).
#
...
# <image-namespace>/ prefix; ACR_REGISTRY is the bare
# registry (<registry>), so FULL_IMAGE = <registry>/<image-namespace>/<name>.
case "$APP" in
  proxy)
    IMAGE_NAME="<image-namespace>/<app-name>-proxy"
