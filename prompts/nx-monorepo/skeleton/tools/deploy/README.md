...
1. Resolves the per-app **image name**, **build context**, and **Dockerfile** from a `case` on the
   app argument:
   - `proxy` → `<image-namespace>/<app-name>-proxy`, context `apps/proxy`, `apps/proxy/Dockerfile`
   - `frontend` → `<image-namespace>/<app-name>-frontend`, context repo root,
     `apps/frontend/Dockerfile` (the SSR/BFF build needs the pnpm workspace root)
   - `backend` → `<image-namespace>/<app-name>-backend`, context `apps/backend`,
...
4. Builds `$ACR_REGISTRY/<image>:$TAG`, logs in (`--password-stdin`), and pushes. `ACR_REGISTRY` is
   the **bare** registry host; the `<image-namespace>/` path is part of `<image>`, so the full ref
   is e.g. `<registry>/<image-namespace>/<app-name>-backend:$TAG`.
